...
\begin{itemize}
  \item \textbf{Two observable overlap statistics}, both computable from a design matrix alone and so available to a practitioner before any rule is chosen. The \emph{mean pairwise overlap} is the mean over facility pairs of $|{\cap}|/|{\cup}|$ of their covariate ranges; across the third pass it tracks the latent design parameter closely (Pearson $r=0.995$). The \emph{minimum fold overlap} is the smallest, over leave-one-facility-out folds, of the same quantity computed between the held-out facility's range and the union of the others'. The decision rule of Section~\ref{sec:results-v3} keys on the second, because a permutation test needs admissible label swaps in \emph{every} fold and one separated facility is enough to remove them from its own; results are binned on it throughout. The first is reported alongside only to show how far the two can diverge on a real corpus.
...
\paragraph{The identifiability limit.} That advantage has a boundary, and the boundary lies inside the lowest bin of Table~\ref{tab:overlap} rather than at its edge. At \emph{exactly} disjoint windows the stratified test degenerates: in a dedicated batch of 150 scenarios per arm at overlap $=0$ (\texttt{degenerate\_corner\_overlap\_zero} in \texttt{results/synth\_scenarios\_v3\_summary.json}) it returns a 0.0\% false-alarm rate and 0.0\% power --- with facility identity a deterministic function of the covariates, each stratum contains a single facility, no label swap is admissible, and the test cannot reject anything. The naive test in that same corner reaches a 20.0\% false-alarm rate, while the threshold rule holds 0.0\% false alarms at 85.3\% power. This is not a defect of the estimator but an identifiability limit: under perfect separation, ``the facilities follow different laws'' and ``the facilities were run at different conditions'' are not distinguishable by any argument based on permuting facility labels. Where the minimum fold overlap approaches zero, the plain threshold rule is the defensible choice, accepting roughly 15 points of power to retain a zero false-alarm rate.
...
\paragraph{Where the companion corpus falls.} Because the choice of rule depends on a quantity that can be measured, we computed both statistics on the companion manuscripts' 114-point windage corpus (a description of its design, not a re-analysis of its windage relationship; \texttt{results/companion\_overlap.json}). Its mean pairwise overlap in $\log Re$ is 0.317, which by itself would suggest the stratified test applies. Its minimum fold overlap is 0.008. The gap between those two numbers is the point: three of the four facilities overlap substantially with one another (pairwise 0.53--0.69), while the largest, contributing 45 of the 114 points, is essentially disjoint from all of them (0.009, 0.000, 0.000), so the fold holding it out sits at the identifiability limit above. On the decision rule we recommend, that corpus is a case for the plain threshold rule rather than for either permutation test --- and the companion studies did use the threshold rule, so their reported verdict is unaffected by any of this. It does mean that a practitioner who read only Sections~\ref{sec:results-v1}--\ref{sec:results-v2}, or who checked the mean pairwise overlap alone, would have reached for a test that is not admissible on that dataset.
