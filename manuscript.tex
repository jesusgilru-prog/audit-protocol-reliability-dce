...
\begin{itemize}
  \item \textbf{Two observable overlap statistics}, both computable from a design matrix alone and so available to a practitioner before any rule is chosen. The \emph{mean pairwise overlap} is the mean over facility pairs of $|{\cap}|/|{\cup}|$ of their covariate ranges; across the third pass it tracks the latent design parameter closely (Pearson $r=0.995$). The \emph{minimum fold overlap} is the smallest, over leave-one-facility-out folds, of the same quantity computed between the held-out facility's range and the union of the others'. A third, \emph{minimum fold point overlap}, replaces the range comparison by the fraction of the held-out facility's points that fall inside the range spanned by the others, again minimised over folds. The decision rule of Section~\ref{sec:results-v3} keys on the last two, because a permutation test needs admissible label swaps in \emph{every} fold: one separated facility removes them from its own, and a range comparison can be lifted off zero by a single boundary observation without any usable support behind it. Results are binned on the range-based minimum, with the point-based statistic used as the check that catches that failure. The mean is reported alongside only to show how far the three can diverge on a real corpus.
...
The recommendation this study supports is a two-step procedure rather than a single rule. \emph{First, measure how much your data sources actually overlap in covariate space}; the statistic is the minimum over leave-one-out folds of the intersection-over-union between the held-out source's covariate range and the union of the others' (Method), it needs nothing but the design matrix, and it decides which of the following applies. Use the minimum rather than an average across pairs: one separated source disables the test for its own fold however well the others agree. \emph{Second, choose the declaration rule accordingly}:
