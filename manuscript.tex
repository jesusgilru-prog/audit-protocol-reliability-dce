...
\abstract{Leave-one-facility-out (LOFO) cross-validation with bootstrap resampling is widely used to audit whether a data-driven scaling law generalizes across experimental sources or is an artifact of one facility. Two companion manuscripts \citep{gilruiz2026paper1,gilruiz2026paper2} applied it to a 114-point windage dataset spanning four facilities and found a good in-sample fit collapsing under LOFO. Whether such a verdict can be trusted at that scale is unanswerable from the dataset itself, so we study the protocol under known synthetic ground truth across three passes totalling 18,000 scenarios. The conventional declaration rule, LOFO R\textsuperscript{2}$>0$, is reliable against parameter drift and heavy-tailed noise but nearly blind to two realistic failure modes, flagging only 9.4\% of nonlinear-misspecified and 42.7\% of clustered confounding, and for the former it degrades further as facilities are added. A facility-label permutation test computed from the same fits recovers most of that loss. Its validity, however, rests on an assumption the first two passes could not expose, because they give every facility identical covariate support: the permutation null requires the sources to overlap in covariate space. As operating windows separate the naive test becomes up to threefold anti-conservative, while permuting labels only within covariate strata restores calibration at little cost in power. Under near-total separation neither works, and the plain threshold rule, which false-alarms on 4 of 3,959 universal second-pass scenarios and on none at all in the overlap sweep, is the defensible fallback. We therefore key the choice to statistics computable from the design matrix alone, and calibrate the criterion: what a permutation needs is that the worst fold share at least one observation with the rest, which the companion corpus does, by a single point.}
...
Stratifying the permutation repairs the problem essentially for free. Restricting label swaps to within-stratum swaps holds the false-alarm rate at 4.4--6.8\% across the whole range --- every bin's 95\% interval covers the nominal 0.05 --- while costing little power: 97.6\% in the lowest bin and 100\% elsewhere, against the threshold rule's 81.8--88.5\%. Across the bins of Table~\ref{tab:overlap} the stratified test is therefore the best of the three on both criteria at once, and it is the rule we recommend wherever it is admissible.

\paragraph{Where the advantage ends.} The lowest bin of Table~\ref{tab:overlap} still shows the stratified test working, so the boundary lies inside it. Two further batches locate it (\texttt{results/overlap\_fine\_sweep.json}, \texttt{results/overlap\_transition.json}). The transition is not gradual but a knife edge at zero. Where every fold retains \emph{some} positive overlap the stratified test keeps full power --- 98.4\% at minimum fold overlap 0.003--0.006, 98.5\% at 0.006--0.010 and 100.0\% at 0.010--0.020 --- whereas once at least one fold is exactly disjoint its power falls to 56.2\% ($n{=}162$), because that fold admits no label swap and contributes nothing while the remaining folds carry the test alone. When \emph{all} facilities are mutually disjoint, the dedicated batch of 150 scenarios per arm at overlap $=0$ (\texttt{degenerate\_corner\_overlap\_zero} in \texttt{results/synth\_scenarios\_v3\_summary.json}) returns 0.0\% false alarms and 0.0\% power: with facility identity a deterministic function of the covariates, every stratum contains a single facility and the test cannot reject anything. The naive test in that same corner reaches a 20.0\% false-alarm rate; the threshold rule holds 0.0\% false alarms at 85.3\% power. The inertia at the limit is not a defect of the estimator but an identifiability property: under perfect separation, ``the facilities follow different laws'' and ``the facilities were run at different conditions'' are not distinguishable by any argument based on permuting facility labels.

\paragraph{How much shared support is enough.} Because the boundary sits at exactly zero, the range statistic is fragile there: a single observation lifts it off zero. Whether that matters is an empirical question, so we measured it rather than argued it. Table~\ref{tab:points} bins scenarios by the number of points of the held-out facility that actually fall inside the range spanned by the others, on the fold where that count is smallest, with 140 scenarios per arm per bin. The answer is that one point is enough. Power is 60.7\% when no point is shared and 100.0\% (95\% CI [0.973, 1.000]) from one point onwards, flat across every larger bin. Calibration is approximately nominal but noisy over the same range, between 2.9\% and 12.1\%, with two of six bins' intervals excluding 0.05 and no monotone trend; we read that as sampling noise around a mildly anti-conservative test rather than as structure.

The operational criterion is therefore whether the worst fold shares \emph{any} support at all, not how much. That is a weaker requirement than we expected, and it is worth stating plainly because two earlier versions of this analysis assumed the opposite (Limitations).
...
The single most actionable recommendation from this study is a two-step procedure rather than a single rule. \emph{First, measure how much your data sources actually overlap in covariate space}; the statistic is the minimum over leave-one-out folds of the intersection-over-union between the held-out source's covariate range and the union of the others' (Method), it needs nothing but the design matrix, and it decides which of the following applies. Use the minimum rather than an average across pairs: one separated source disables the test for its own fold however well the others agree. \emph{Second, choose the declaration rule accordingly}:
...
\begin{itemize}
  ...
  \item The number of each facility's points lying inside the $\log Re$ range spanned by the other three, per leave-one-facility-out fold, is 1 of 45 (Guo), 20 of 20 (Liu), 31 of 41 (Vrancik) and 8 of 8 (Zheng).
\end{itemize}
