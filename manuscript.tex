...
Stratifying the permutation repairs the problem essentially for free. Restricting label swaps to within-stratum swaps holds the false-alarm rate at 4.4--6.8\% across the whole range --- every bin's 95\% interval covers the nominal 0.05 --- while keeping power at 97.6\% or above, against the threshold rule's 81.8--88.5\%. Across the bins of Table~\ref{tab:overlap} the stratified test is therefore the best of the three on both criteria at once, and it is the rule we recommend wherever it is admissible.
...
\begin{itemize}
  ...
  \item \textbf{Minimum fold overlap near zero}: use the plain LOFO$>0$ threshold. It detects less (roughly 15 points less power), but it is the only rule in our sweep that never produces a false alarm, and in this regime no permutation-based alternative is trustworthy. The companion corpus falls here (Section~\ref{sec:results-v3}), despite a mean pairwise overlap that would suggest otherwise.
\end{itemize}
