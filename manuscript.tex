...
\paragraph{Endpoints are not support.} That the boundary sits at exactly zero makes the range-based statistic fragile there, because a single observation can lift it off zero without providing usable support. We therefore also report, per fold, the \emph{fraction of held-out points} lying inside the range spanned by the other facilities, and recommend checking it alongside. The two statistics agree except near the limit, and the companion corpus below is precisely a case where they do not.
...
\paragraph{Where the companion corpus falls.} Because the choice of rule depends on quantities that can be measured, we computed them on the companion manuscripts' 114-point windage corpus (a description of its design, not a re-analysis of its windage relationship; \texttt{results/companion\_overlap.json}). Its mean pairwise overlap in $\log Re$ is 0.317, which taken alone would suggest a comfortable margin. Its minimum fold overlap is 0.008, which on the range statistic sits just above the transition identified above and would still admit the stratified test. The point-based check disagrees, and it is the one to believe here: the shared region for the fold holding out the largest facility spans $\log_{10} Re \in [6.796, 6.819]$, and contains exactly one of that facility's 45 points and one of the other 69. Its range overlap is non-zero only because two boundary observations happen to fall 0.023 decades apart. On that fold a stratified permutation has essentially no admissible swaps, so the corpus behaves like the exactly-disjoint case rather than like the positive-overlap case its range statistic suggests.

The conservative reading is therefore the appropriate one: on this corpus the plain threshold rule is the defensible choice, and neither permutation variant should be relied on for the fold that holds out the largest facility. The companion studies did use the threshold rule, so their reported verdict is unaffected. Two things follow for practice. A reader who checked only the mean pairwise overlap would have reached for a test that is not usable here; and a reader who checked only the range-based minimum would have reached the same wrong conclusion for a subtler reason. All three statistics are one line of code each, and this corpus is a case where only the third gives the right answer.

Note also what this does \emph{not} say. The obstruction is a property of how these four facilities were operated, not of the audit protocol or of the windage question; a corpus covering the same four facilities over overlapping Reynolds windows would be testable by the stratified rule with the power reported above. The limitation is in the data collection, and it is the kind that is cheap to avoid when a corpus is being assembled and impossible to repair afterwards.
...
\begin{itemize}
  \item \textbf{Every fold has support the permutation can use}: use the \emph{stratified} facility-label permutation test. Check both the range-based minimum and the fraction of held-out points inside the others' range, since the first alone can be lifted off zero by one boundary observation. It is computed from the same fits a LOFO audit already produces, requires no additional data collection, closes most of the gap between the threshold rule's best mechanism (Gaussian drift, 82.3\% detection) and its worst (nonlinear misspecification, 9.4\% detection), and unlike the naive permutation test it stays calibrated as the sources separate (Table~\ref{tab:overlap}).
  \item \textbf{Some fold has no usable shared support}: use the plain LOFO$>0$ threshold. It detects less (roughly 15 points less power), but it is by far the most conservative of the three --- no false alarm at any overlap level in the third pass, and 4 in 3,959 universal scenarios across the second --- and a stratified test loses roughly 44 points of power on the fold in question. The companion corpus falls here (Section~\ref{sec:results-v3}), despite both a mean pairwise overlap and a range-based minimum that would suggest otherwise.
\end{itemize}
